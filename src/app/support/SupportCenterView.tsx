 ...
 * 支持中心的纯展示组件，被两个入口复用：
 *
 * - `src/app/support/page.tsx`：动态渲染（frontend-server 等 SSR 边界），
 *   用 headers() 判定语言，拉取真实文档服务数据。
 * - `static-dashboard/app/support/page.tsx`：静态导出边界（output:"export"），
 *   不能调用 headers()，也不能调用内部会调 headers() 的文档服务客户端
 *   （原先 /docs 正因为要用这些能力才没被纳入 static-dashboard），
 *   只能用构建期已知的静态兜底内容渲染同一份视觉。
 *
 * 两条入口算出的数据形状完全一致，所以把 JSX 抽到这里，靠 props 区分来源，
 * 而不是靠运行时判断——避免又在这个组件里悄悄调用一次 headers()。
 */
...
import type { DocCollectionPayload } from "@/lib/docsServiceClient";
...
export function toProductCollections(
...
export type DocNavGroup = {
  label: string;
  links: Array<{ title: string; href?: string; active: boolean }>;
};
...
export function SupportCenterView({
  ...
  collections: DocCollectionPayload[];
  firstCollectionHref: string;
  articleCount: number;
  featuredArticle: FeaturedArticle;
  docNavGroups: DocNavGroup[];
}) {
