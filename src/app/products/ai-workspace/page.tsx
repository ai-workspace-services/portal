...
import { ArrowRight, Bot, Cloud, Network, ShieldCheck, Lock, Layers, Activity, Monitor } from "lucide-react";
import Link from "next/link";
import { useLanguage } from "@/i18n/LanguageProvider";
import aiWorkspaceContent from "@/data/content/ai-workspace.json";
...
export default function AiWorkspacePage() {
  ...
  const content = (aiWorkspaceContent as any)[language] || (aiWorkspaceContent as any).zh;
    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <main className="pt-24 pb-16 sm:pt-32">
        ...
        <section className={`${marketingTheme.section.container} text-center`}>
          <div className="inline-flex items-center rounded-full border border-sky-500/30 bg-sky-50/50 px-3 py-1 text-sm font-medium text-sky-600 mb-8">
            ...
            return (
              <div key={idx} className={`flex flex-col ${isReverse ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center gap-12`}>
                <div className={`flex-1 ${isReverse ? 'lg:pl-10' : 'lg:pr-10'}`}>
                  <div className="mb-6 inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-sky-100 text-sky-600 shadow-inner">
                ...
                <div className="flex-1 w-full relative">
                  <div className={`absolute -inset-4 bg-gradient-to-${isReverse ? 'tl' : 'tr'} from-sky-100 to-indigo-50 opacity-50 blur-xl rounded-full`} />
