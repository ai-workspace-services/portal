...
export default function OpsAccountTriage() {
  ...
  const usersSWR = useSWR<ManagedUser[]>(canAccess ? `${ADMIN_API_BASE}/billing/accounts` : null, jsonFetcher, {
  ...
  return (
    <div className="space-y-5 pb-8">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <p className="text-eyebrow font-semibold uppercase tracking-[0.18em] text-[var(--color-primary)]">Operations / account triage</p>
              ...
              <tbody className="divide-y divide-[color:var(--color-surface-border)]">
                ...
                {!usersSWR.isLoading ? filteredRows.slice(0, 10).map((row) => {
                  ...
                  return <tr key={row.id} className={`cursor-pointer transition hover:bg-[var(--color-surface-hover)] ${isSelected ? "bg-[var(--color-primary-muted)]/45" : ""}`} onClick={() => setSelectedId(row.id)}>
                    <td className="px-4 py-3 sm:px-5"><span className={`inline-flex whitespace-nowrap rounded-full border px-2 py-1 text-eyebrow font-medium ${stateClass(row.state)}`}>{stateLabel(row.state)}</span></td>
        ...
        <aside className={`${surface} p-4 sm:p-5`} aria-labelledby="account-preview-heading">
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0"><h2 id="account-preview-heading" className="truncate text-base font-semibold text-[var(--color-heading)]">{detailUser ? displayName(detailUser) : "选择一个账号"}</h2><p className="truncate text-xs text-[var(--color-text-subtle)]">{detailUser?.email ?? "从左侧列表选择账号"}</p><p className="mt-1 truncate font-mono text-eyebrow text-[var(--color-text-subtle)]">UUID: {detailUser?.id ?? "—"}</p></div>
            </div>
          ...
          <div className="mt-5 border-t border-[color:var(--color-surface-border)] pt-4">
            <div className="flex items-center justify-between"><h3 className="text-sm font-semibold text-[var(--color-heading)]">订阅与套餐</h3><span className={`rounded-full px-2 py-1 text-eyebrow font-medium ${stateClass(selectedUser?.state ?? "syncing")}`}>{stateLabel(selectedUser?.state ?? "syncing")}</span></div>
...
function OverviewMetric({ icon: Icon, label, value, hint, tone }: { icon: LucideIcon; label: string; value: string; hint: string; tone: "blue" | "green" | "red" | "orange" }) {
  ...
  return <section className={`${surface} p-4`}><div className="flex items-start justify-between"><div><p className="text-xs text-[var(--color-text-muted)]">{label}</p><p className="mt-2 text-2xl font-semibold tracking-tight text-[var(--color-heading)]">{value}</p><p className="mt-2 text-eyebrow text-[var(--color-text-subtle)]">{hint}</p></div><span className={`flex h-10 w-10 items-center justify-center rounded-xl ${toneClass}`}><Icon className="h-5 w-5" /></span></div></section>;
}
