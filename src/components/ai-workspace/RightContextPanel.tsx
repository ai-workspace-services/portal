...
import { Copy, RefreshCw, PanelRightClose, PanelRightOpen, MessageSquare, Folder, FileText } from 'lucide-react';
...
export default function RightContextPanel() {
  ...
  return (
    <aside 
      className={cn(
        "bg-white border-l border-gray-100 flex flex-col h-full flex-shrink-0 transition-all duration-300 ease-in-out",
        collapsed ? "w-12" : "w-80"
      )}
    >
      {collapsed ? (
        <div className="flex flex-col items-center py-4 h-full gap-4">
          <button onClick={() => setCollapsed(false)} className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-md transition-colors" title="展开上下文">
            <PanelRightOpen className="w-4 h-4" />
          </button>
          <div className="flex-1 flex flex-col items-center gap-4 mt-4">
            <div className="p-2 bg-gray-50 rounded-lg text-gray-400" title="上下文会话">
              <MessageSquare className="w-4 h-4" />
            </div>
            <div className="p-2 bg-gray-50 rounded-lg text-gray-400" title="文件">
          ...
          <div className="p-4 border-b border-gray-100">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-1 text-gray-400">
                <button className="p-1.5 hover:bg-gray-100 rounded-md transition-colors"><RefreshCw className="w-4 h-4" /></button>
                <button onClick={() => setCollapsed(true)} className="p-1.5 hover:bg-gray-100 rounded-md transition-colors"><PanelRightClose className="w-4 h-4" /></button>
              </div>
            </div>
